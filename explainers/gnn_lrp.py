...
class GNN_LRP(WalkBase):
    ...
    def forward(self,
        ...
        # self.num_edges = edge_index_with_loop.shape[1] - self.num_nodes
        ...
        if not self.explain_graph:
            ...
            self.subset, _, _, self.hard_edge_mask = k_hop_subgraph(
                node_idx, self.__num_hops__, edge_index_with_loop, relabel_nodes=True,
                flow=self.__flow__(), directed=True)
        ...
        if kwargs.get('walks'):
            ...
            for label in labels:
                ...
                compute_walk_score()
                if kwargs.get('target') is not None:
                    walk_scores_tensor_list[0] = torch.stack(walk_scores, dim=0).view(-1, 1)
                else:
            ...
            with self.connect_mask(self):
                ...
                for ex_label in ex_labels:
                    ...
                    if self.explain_graph:
                        self.hard_edge_mask = torch.ones_like(edge_mask, dtype=torch.bool)
                    edge_mask[self.hard_edge_mask] -= edge_mask[self.hard_edge_mask].min()
                    edge_mask[self.hard_edge_mask] /= edge_mask[self.hard_edge_mask].max() + EPS
                    edge_mask[self.hard_edge_mask] += EPS
                    edge_mask[~self.hard_edge_mask] = 0
                ...
                # related_preds = self.eval_related_pred(x, edge_index, hard_edge_masks, node_idx=node_idx)

        return walks, edge_masks
