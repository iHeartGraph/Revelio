...
class SubgraphX(object):
    ...
    def explain(self, x: Tensor, edge_index: Tensor, label: [int | Tensor],
                max_nodes: int = 5,
        ...
        node_mask = torch.zeros(self.ori_data.x.shape[0], device=self.device)
    ...
    def __call__(self, x: Tensor, edge_index: Tensor, **kwargs) \
        ...
        for label_idx, label in enumerate(ex_labels):
            results = self.explain(x, edge_index,
                                   label=label,
                                   max_nodes=max_nodes,
                                   node_idx=node_idx,
