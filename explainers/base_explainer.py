...
class ExplainerBase(nn.Module):
    ...
    def __subgraph__(self, node_idx: int, x: Tensor, edge_index: Tensor, **kwargs):
        ...
        subset, edge_index, mapping, edge_mask = k_hop_subgraph(
            ...
            num_nodes=num_nodes, flow=self.__flow__(), directed=True)
    ...
    def batch_input(self, x, edge_index, batch_size):
        ...
        batch_batch = torch.arange(batch_size, device=self.device).unsqueeze(1).repeat(1, num_nodes).view(-1)
        batch_edge_batch = torch.arange(batch_size, device=self.device).unsqueeze(1).repeat(1, num_edges).view(1, -1)
...
class WalkBase(ExplainerBase):
    ...
    def explain_edges_with_loop(self, x: Tensor, walks: Dict[Tensor, Tensor], ex_label):
        ...
        idx_ensemble = torch.cat([(walks_ids == i).int().sum(dim=1).unsqueeze(0) for i in range(self.num_edges)],
                                     dim=0)
    ...
    class connect_mask(object):
        ...
        def __enter__(self):
            ...
            self.cls.edge_mask = [nn.Parameter(torch.randn(self.cls.x_batch_size * self.cls.num_edges)) for _ in
                             range(self.cls.num_layers)] if hasattr(self.cls, 'x_batch_size') else \
                                 [nn.Parameter(torch.randn(1 * self.cls.num_edges)) for _ in
    ...
    class temp_mask(object):
        ...
        def __enter__(self):
            ...
            for idx, module in enumerate(self.cls.mp_layers):
                module.explain = True
                module._edge_mask = self.temp_edge_mask[idx]
                module._apply_sigmoid = False
