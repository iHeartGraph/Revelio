    ...
    def forward(self,
        ...
        if kwargs.get('edge_masks'):
            ...
            for ex_label in ex_labels:
                ...
                if self.explain_graph:
                    self.hard_edge_mask = torch.ones_like(score_mask, dtype=torch.bool)
                score_mask[self.hard_edge_mask] -= score_mask[self.hard_edge_mask].min()
                score_mask[self.hard_edge_mask] /= score_mask[self.hard_edge_mask].max() + EPS
                score_mask[self.hard_edge_mask] += EPS
                score_mask[~self.hard_edge_mask] = 0
