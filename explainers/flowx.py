...
from torch_geometric.utils.loop import add_self_loops, add_remaining_self_loops, remove_self_loops
...
class FlowX(WalkBase):
    ...
    def forward(self,
        ...
        edge_index_with_loop = edge_index
        ...
        if not self.explain_graph:
            ...
            _, _, _, self.hard_edge_mask = k_hop_subgraph(
                node_idx, self.__num_hops__, edge_index_with_loop, relabel_nodes=True,
                num_nodes=None, flow=self.__flow__(), directed=True)
        ...
        if kwargs.get('target') is not None:
            labels = tuple([kwargs.get('target')])
        else:
        ...
        for ex_label in ex_labels:
            if kwargs.get('target') is not None:
                edge_attr = self.explain_edges_with_loop(x, walks, 0)
            else:
                edge_attr = self.explain_edges_with_loop(x, walks, ex_label)
            mask = edge_attr
            if self.explain_graph:
                self.hard_edge_mask = torch.ones_like(mask, dtype=torch.bool)
            mask[self.hard_edge_mask] -= mask[self.hard_edge_mask].min()
            mask[self.hard_edge_mask] /= mask[self.hard_edge_mask].max() + EPS
            mask[self.hard_edge_mask] += EPS
            mask[~self.hard_edge_mask] = 0
            # mask[~self.hard_edge_mask] = torch.tensor(-float('inf'))
    ...
    def train_mask(self,
        ...
        self.flow2layeredge_matrix = torch.stack([(walk_plain_indices_list == i).float().sum(dim=1)
                                                  for i in
                                                  range(self.num_layers * self.num_edges)],
                                                 dim=1).detach()
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if kwargs.get('target') is not None:
                self.flow_mask = shap_flow_score[:, 0].unsqueeze(1)
            else:
    ...
    def flow_shap(self,
        ...
        for iter_idx in range(self.ns_iter):
            ...
            walk_sample_count += (weighted_change_walks_list > 1e-30).float().sum(0).detach()
