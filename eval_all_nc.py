...
for explainer in explainers:
    ...
    pbar = tqdm(total=candidates, desc=explainer.capitalize())
    for node_index in node_ids:
        ...
        if explainer in ['ours', 'gnn-lrp', 'flowx']:
            if args.fidelity_plus and explainer != 'gnn-lrp':
                flows = torch.load(os.path.join(res_dir, explainer + '_plus_' + str(node_index) + '.pt'),
                                   map_location='cpu')
            else:
                flows = torch.load(os.path.join(res_dir, explainer + '_' + str(node_index) + '.pt'),
                                   map_location='cpu')
            edge_mask = flows['mask']
        else:
            if args.fidelity_plus and explainer not in ['gradcam', 'deeplift']:
                edge_mask = torch.load(os.path.join(res_dir, explainer + '_plus_' + str(node_index) + '.pt'),
                                       map_location='cpu')
            else:
                edge_mask = torch.load(os.path.join(res_dir, explainer + '_' + str(node_index) + '.pt'),
                                       map_location='cpu')
